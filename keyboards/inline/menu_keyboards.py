...
sure_choise = CallbackData("sure", "item_id")
...
async def categories_keyboard():
    ...
    # Забираем список из базы данных с РАЗНЫМИ категориями и проходим по нему
...
async def items_keyboard(category, subcategory):
    ...
    for item in items:
        ...
        markup.insert(
            InlineKeyboardButton(
                text=button_text, callback_data=sure_choise.new(item_id=item.id)) #text=button_text, callback_data=callback_data) для нового ЛВЛа
        )
...
# Создаем функцию, которая отдает клавиатуру с кнопками "купить" и "назад" для выбранного товара Это для нового ЛВЛа
...
#         InlineKeyboardButton(
#             text=f"Sure",
#             callback_data=sure_choise.new(item_id=item_id)
#         )
#     )
#     markup.row(
#         InlineKeyboardButton(
